 ...
 * Fix the bug while swiping from END to Start, the view's alpha not changing
 * Fix the bug if undo after the swiping, the child view is not showing up because of the alpha is 0f
 */
public class CustomSwipeDismissBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    private final ViewDragHelper.Callback mDragCallback = new ViewDragHelper.Callback() {
        ...
        public void onViewPositionChanged(View child, int left, int top, int dx, int dy) {
            final float leftStartAlphaDistance = mOriginalCapturedViewLeft
                    - child.getWidth() * mAlphaStartSwipeDistance;
            final float leftEndAlphaDistance = mOriginalCapturedViewLeft
                    - child.getWidth() * mAlphaEndSwipeDistance;
            final float rightStartAlphaDistance = mOriginalCapturedViewLeft
                    + child.getWidth() * mAlphaStartSwipeDistance;
            final float rightEndAlphaDistance = mOriginalCapturedViewLeft
                    + child.getWidth() * mAlphaEndSwipeDistance;

            if (left >= leftStartAlphaDistance && left <= rightStartAlphaDistance) {
                ViewCompat.setAlpha(child, 1f);
            } else if (left <= leftEndAlphaDistance || left >= rightEndAlphaDistance) {
                ViewCompat.setAlpha(child, 0f);
            } else {
                // We're between the start and end distances
                if (left < leftStartAlphaDistance) {
                    // Swiping from End to Start
                    final float leftDistance = fraction(leftStartAlphaDistance, leftEndAlphaDistance, left);
                    ViewCompat.setAlpha(child, clamp(0f, 1f - leftDistance, 1f));
                } else {
                    // Swiping from Start to End
                    final float rightDistance = fraction(rightStartAlphaDistance, rightEndAlphaDistance, left);
                    ViewCompat.setAlpha(child, clamp(0f, 1f - rightDistance, 1f));
                }
            }
        }
    };
